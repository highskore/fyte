...
contract Match_Fuzz_Test is Base_Test {
    ...
    function testFuzz_Hp(uint256 _matchData, uint256 _hp) public pure {
        vm.assume(_hp < 32);
        uint256 updatedMatchData = _matchData.setHp(_hp);
        assert(updatedMatchData.getHp() == _hp);
    }

    function TestFuzz_Round(uint256 _matchData, uint256 _round) public pure {
        vm.assume(_round < 127);
        uint256 updatedMatchData = _matchData.setRound(_round);
        assert(updatedMatchData.getRound() == _round);
    }

    function testFuzz_X(uint256 _matchData, uint256 _x) public pure {
        vm.assume(_x < 32);
    ...
    function testFuzz_Y(uint256 _matchData, uint256 _y) public pure {
        vm.assume(_y < 16);
    ...
    function testFuzz_Direction(uint256 _matchData, uint256 _direction) public pure {
    ...
    function testFuzz_Action(uint256 _matchData, uint256 _action) public pure {
    ...
    function testFuzz_Hitbox(uint256 _matchData, uint256 _hitbox) public pure {
        vm.assume(_hitbox < 64);
        uint256 updatedMatchData = _matchData.setHitbox(_hitbox);
        assert(updatedMatchData.getHitbox() == _hitbox);
    }

    function testFuzz_Combo(uint256 _matchData, uint256 _combo) public pure {
        vm.assume(_combo < 2);
        uint256 updatedMatchData = _matchData.setCombo(_combo);
        assert(updatedMatchData.getCombo() == _combo);
    }

    function testFuzz_Turn(uint256 _matchData, uint256 _turn) public pure {
        vm.assume(_turn < 4);
